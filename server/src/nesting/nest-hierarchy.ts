...
export class NestHierarchy<t extends NestHierarchy<t>> {
    ...
    getActiveChildNode(): t | undefined {
        if (
            ...
            (this.children[this.children.length - 1] as NestHierarchy<t>).end == 0
        ) {
    ...
    addChild(child: t) {
        ...
        const firstChildIndex = sortedIndex(
            this.children,
            child,
            (a, b): boolean => a.start < b.start
        );
        //see how many children will become children of this child:
        //find the last child to become grandchild
        const lastChildIndex = sortedIndex(
            this.children,
            child,
            (a, b): boolean => a.end < b.end,
            firstChildIndex
        );
        child.children = this.children.splice(
            firstChildIndex,
            lastChildIndex + 1 - firstChildIndex,
            child
        );
    }
    addNestedChild(child: t) {
        ...
        while (true) {
            ...
            if (
                currentChildChild &&
                currentChildChild.start <= child.start &&
                currentChildChild.end >= child.end
            )
