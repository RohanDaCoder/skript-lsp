...
export class SkriptFolder extends SkriptFolderContainer {
    ...
    invalidate() {
        for (const file of this.files) file.invalidate();

        for (const folder of this.children) folder.invalidate();
    }
    ...
    async validateRecursively(endFile?: SkriptFile): Promise<boolean> {
        ...
        this.scope = isAddonFolder
            ? new Scope(undefined)
            : this.parent instanceof SkriptFolder
              ? this.parent.scope
              : new Scope(this.parent.getScope());
        ...
        for (const file of this.files) {
            ...
            if (!file.validated) await file.validate();
            this.scope.merge(file.scope);

            if (file === endFile)
                //we don't need patterns after this
                return true;
        }
    ...
    createFoldersForUri(uri: URI): SkriptFolder {
        console.log(
            "creating folder for uri: " + uri.toString() + " , folder uri:" + this.uri.toString()
        );
        ...
        if (child) {
            ...
            if (offset == -1) {
                ...
                const newChild = new SkriptFolder(
                    this,
                    addToUri(this.uri, relativePath.substring(0, offset))
                );
                this.children.splice(
                    this.getPreferredSkriptFileIndexByUri(newChild.uri),
                    0,
                    newChild
                );
