...
export class SkriptWorkSpace extends SkriptFolderContainer {
    ...
    constructor() {
        ...
        this.children.push(
            (this.addonFolder = new SkriptFolder(this, URI.from({ scheme: "internal" })))
        );
    }
    ...
    getSkriptFileByUri(uri: URI): SkriptFile | undefined {
        ...
        if (f) {
            ...
            return (
                this.looseFiles.find((val) => val.uri.toString() == uri.toString()) ??
                this.addonFolder.getSkriptFileByUri(uri)
            );
        }
    }
    ...
    invalidateDependents(file: SkriptFile) {
        ...
        if (file.parent instanceof SkriptFolder) {
            ...
            if (file.parent == this.addonFolder) {
                ...
                for (const looseFile of this.looseFiles) looseFile.invalidate();
            }
        } else file.invalidate();
    }
