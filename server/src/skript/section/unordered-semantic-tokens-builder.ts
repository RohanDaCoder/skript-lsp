...
export class SemanticToken {
    static modToFlags(modifiers: TokenModifiers[]) {
        let flag = 0;
        for (const mod of modifiers) flag |= 1 << mod;
        return flag;
    }
    position: Position;
    length: number;
...
export class SemanticTokenLine {
    ...
    fixTokens() {
        //sort tokens
        this.tokens.sort((a, b) => a.position.character - b.position.character);
    }
        ...
        if (token.length > 0) {
            ...
            if (checkTokens) {
                ...
                for (const lineToken of lineTokens) {
                    if (
                        token.position.character + token.length > lineToken.position.character &&
...
export class UnOrderedSemanticTokensBuilder {
    ...
    push(token: SemanticToken): void {
        if (token.position.line >= this.lines.length) {
            throw new Error(
                token.position.line +
                    " is out of bounds of the file (has " +
                    this.lines.length +
                    " lines )"
            );
        }
    ...
    addLine(line: SemanticTokenLine): void {
        if (line.tokens.length) {
            ...
            if (!existingLine) this.lines[line.tokens[0].position.line] = line;
            else for (const token of line.tokens) existingLine.push(token);
        }
    }
                ...
                this.lines[i].tokens.forEach(
                    (token) =>
                        this._builder.push(
                            ...
                            token.length,
                            token.type,
                            token.tokenModifierFlags
                        ) // 1 << (modifierCounter++ % TokenModifiers.length))//(1 << TokenModifiers.length) - 1)
                );
            }
        }
