...
const sectionRegExp =
    /(aliases|executable by|prefix|usage|description|permission(?: message|)|cooldown(?: (?:message|bypass|storage))?)/;
export class SkriptCommandSection extends SkriptSection {
    ...
    constructor(parent: SkriptSection, context: SkriptContext) {
        ...
        const commandSenderData = this.getScope()?.getPatternMatch(
            new SkriptPatternCall("command sender", [PatternType.expression])
        )?.fullMatch.matchedPattern;
        if (playerType && commandSenderData)
            this.scope.addPattern(
                new PatternData(
                    "[the] player",
                    "(the )?player",
                    commandSenderData.definitionLocation,
                    PatternType.expression,
                    undefined,
                    [],
                    [],
                    new SkriptTypeState(playerType)
                )
            );
        ...
        while ((m = typeRegex.exec(context.currentString))) {
            ...
            this.scope.addPattern(
                new PatternData(
                    "arg[ument]( |-)" + argumentIndex,
                    "arg(ument)?( |-)" + argumentIndex,
                    context.getLocation(typeStart, m[1].length),
                    PatternType.expression,
                    undefined,
                    [],
                    [],
        ...
        if (result == null) {
            if (context.currentString == "trigger")
                ...
                context.addDiagnostic(
                    ...
                    "cannot recognize this section. make sure to put your code for the command in triggers"
                );
                return undefined;
            }
        } else
            context.addDiagnostic(
                ...
                "the " +
                    ...
                    " section has to be in one line. for example " +
                    context.currentString +
                    ": blahblahblah"
            );
    ...
    processLine(context: SkriptContext): void {
        ...
        if (result == null) {
            context.addDiagnostic(
                0,
                context.currentString.length,
                "make sure to put your code for the command in triggers"
            );
        } else context.addToken(TokenTypes.keyword, 0, result[0].length);
    }
}
