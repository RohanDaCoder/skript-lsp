    ...
    constructor(parent: SkriptSection, context: SkriptContext) {
        ...
        if (result == null) {
            context.addDiagnostic(
                ...
                "cannot recognize function",
                DiagnosticSeverity.Error,
                "IntelliSkript->Function->Unrecognized"
            );
            this.name = "";
        } else {
            ...
            if (result[2].trim().length > 0) {
                ...
                for (const currentArgumentString of argumentStrings) {
                    ...
                    if (variableDefinitionParts.length == 2) {
                        ...
                        if (noSpaceResult) {
                            const typeStartPosition =
                                currentArgumentString.index +
                                variableDefinitionParts[0].length +
                                ":".length +
                                noSpaceResult.index;
                            /**where the arguments are located */
                            const loc = specializedContext.getLocation(
                                currentArgumentString.index,
                                variableDefinitionParts[0].trim().length
                            );
                            const initialType = this.parseTypes(
                                specializedContext,
                                typeStartPosition,
                                variableDefinitionParts[1].length
                            );
                        ...
                        specializedContext.addDiagnostic(
                            currentArgumentString.index,
                            currentArgumentString.text.length,
                            'unrecognized function argument (no ":" found)'
                        );
                    }
                }
            }
            ...
            this.pattern = new PatternData(
                ...
                regexPattern,
                context.getLocation(),
                returnTypes.length ? PatternType.expression : PatternType.effect,
                undefined,
                argumentTypes,
                argumentPositions,
                new SkriptTypeState(...returnTypes)
            );
        }
    }
}
