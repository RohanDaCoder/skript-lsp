...
export class ReflectPropertySection extends ReflectExpressionSection {
    ...
    override addPattern(context: SkriptContext): void {
        ...
        if (p) {
            ...
            const p1 = new PatternData(
                "%'s " + p.skriptPatternString,
                "%'s " + p.regexPatternString,
                p.definitionLocation,
                PatternType.expression,
                this,
                [typeState, ...p.expressionArguments]
            );
            const p2 = new PatternData(
                p.skriptPatternString + " of %",
                p.regexPatternString + " of %",
                p.definitionLocation,
                PatternType.expression,
                this,
                [...p.expressionArguments, typeState]
            );
