...
export class ReflectPatternContainerSection extends SkriptSection {
    ...
    parsePattern(context: SkriptContext): PatternData | undefined {
        const pattern = PatternTree.parsePattern(
            context,
            this,
            (<typeof ReflectPatternContainerSection>this.constructor).patternType
        );
        if (pattern) {
            ...
            if (this.patterns.length == 0) {
                ...
                for (const argumentType of pattern.expressionArguments) {
                    ...
                    counter++;
                    context.parseResult.newPatterns.push([
                        this.scope,
                        new PatternData(
                            "expr-" + counter,
                            "expr-" + counter,
    ...
    processLine(context: SkriptContext): void {
        ...
        if (result) {
            ...
            context.addDiagnostic(
                ...
                "expected patterns here",
                DiagnosticSeverity.Error
            );
        }
    }
    override finish(context: SkriptContext) {
        for (const pattern of this.patterns) context.currentSkriptFile.addPattern(pattern);
