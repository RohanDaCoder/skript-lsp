...
import {
    SemanticTokenLine,
    UnOrderedSemanticTokensBuilder,
} from "./unordered-semantic-tokens-builder";
...
export class SkriptFile extends SkriptSection {
    ...
    createSection(context: SkriptContext): SkriptSection | undefined {
        ...
        const sectionKeyword =
            spaceIndex == -1
                ...
                : context.currentString.substring(0, spaceIndex);
        ...
        if (sectionKeyword == "function") {
            ...
            if (result) {
                ...
                const propertyResult =
                    /^(?:((?:(?:local) )?(?:(?:plural|non-single) )?)((?:[^\s]| ){1,}) property) .*/.exec(
                        context.currentString
                    );
                if (propertyResult) {
        ...
        if (addKeywordToken) context.addToken(TokenTypes.keyword, 0, sectionKeyword.length);
        return s;
    }
    ...
    processLine(context: SkriptContext): void {
        context.addDiagnostic(
            ...
            "can't understand this line (colon or indentation missing?"
        );
    }
    ...
    static trimLineWithoutComments(line: string): { trimmedLine: string; commentIndex: number } {
        ...
        for (let i = 0; i < line.length; i++) {
            ...
            if ('"%'.includes(char)) {
                if (
                    currentDelimiters.length > 0 &&
                    currentDelimiters[currentDelimiters.length - 1] == char
                ) {
                    currentDelimiters = currentDelimiters.substring(
                        0,
                        currentDelimiters.length - 1
                    );
                } else currentDelimiters += line[i];
    ...
    static validateCodeLine(
        ...
        if (indentData.hasColon) {
            //indent
            sectionContext.parseResult.newSection =
                sectionContext.currentSection.createSection(sectionContext);
            if (sectionContext.parseResult.newSection == undefined) {
                sectionContext.addDiagnostic(
                    0,
                    sectionContext.currentString.length,
                    "can't recognise this section. (pattern detection is a work in progress. please report on discord)",
                    DiagnosticSeverity.Hint,
                    "IntelliSkript->Section->Not Recognised"
                );
            }
        } else {
    ...
    async validate() {
            ...
            if (stacksToPop > 0) {
                ...
                if (startLine == currentLineIndex - 1) {
                    context.addDiagnosticAbsolute(
                        ...
                        "empty section (expected something here)",
                        DiagnosticSeverity.Warning,
                        "IntelliSkript->Indent->Empty"
                    );
                }
        ...
        while (currentLineIndex < terminatedLines.length) {
            ...
            const currentLine =
                terminatorIndex == -1
                    ? currentLineTerminated
                    : currentLineTerminated.substring(0, terminatorIndex);
            ...
            if (inMultiLineComment || wasInMultiLineComment) {
                ...
                if (trimmedLine.length > 0) {
                    ...
                    const trimmedContext = currentLineContext.push(
                        indentData.endIndex,
                        trimmedLine.length
                    );
                    ...
                    const checkSection = (section: SkriptSection): boolean => {
                        ...
                        try {
                            validatedContext = SkriptFile.validateCodeLine(
                                trimmedContext,
                                section,
                                indentData
                            );
                        ...
                        const parsedCorrectly =
                            validatedContext.parseResult.diagnostics.length == 0;
                        ...
                        if (
                            ...
                            mostValidContext = validatedContext;
                    ...
                    for (let i = 0; i < expectedStacksToPop; i++) {
                        ...
                        if (parent) expectedSection = parent;
                        else {
                    ...
                    if (!checkSection(expectedSection)) {
                        ...
                        const passedTypes = new Set<string>([
                            ReflectPatternSection.name,
                            expectedSection.constructor.name,
                        ]);
                    ...
                    if (mostValidContext) {
                        ...
                        this.builder.addLine(
                            mostValidContext.parseResult.tokens as SemanticTokenLine
                        );
                        this.parseResult.diagnostics.push(
                            ...mostValidContext.parseResult.diagnostics
                        );
                        ...
                        if (indentData.hasColon) {
                            ...
                            const newSection =
                                mostValidContext.parseResult.newSection ??
                                new SkriptSection(
                                    mostValidContext.currentSection,
                                    mostValidContext
                                );
                        ...
                        this.parseResult.frequencyMatrix.push(
                            mostValidContext.parseResult.frequencyMatrix
                        );
                        indentData.finishLine();
                        if (indentData.current != indentData.correct) {
                            currentLineContext.addDiagnostic(
                                0,
                                indentData.current,
                                "this line works when adjusting indentation. press ctrl + F to fix",
                    ...
                    for (
                        let suggestIndex = lastCodeLine + 1;
                        suggestIndex <= currentLineIndex;
                        suggestIndex++
                    ) {
                ...
                if (trimInfo.commentIndex != -1) {
                    currentLineContext.addToken(
                        TokenTypes.comment,
                        trimInfo.commentIndex,
                        currentLine.length - trimInfo.commentIndex
                    );
                }
            }
            currentLineIndex++;
                ...
                edits.push(
                    TextEdit.replace(
                        Range.create(
                            ...
                            { line: index, character: currentIndentation.length }
                        ),
                        recommendedIndentation
                    )
                );
            }
            if (trimInfo.commentIndex != -1) {
                ...
                if (charBehindHashtag != undefined && charBehindHashtag.match(/\S/)) {
                    edits.push(
                        TextEdit.insert({ line: index, character: trimInfo.commentIndex + 1 }, " ")
                    );
                }
            }
        }
    ...
    getLineContext(lineIndex: number): SkriptContext {
        ...
        const context = new SkriptContext(
            this,
            this.text.substring(lineRange.start, lineRange.end)
        );
