...
export class PatternTree {
    ...
    separateNodeChildren(
        oldChildren: Map<string, PatternTreeNode>,
        newChildren: Map<string, PatternTreeNode>
    ) {
    ...
    addPatternPart(
        data: PatternData,
        currentNodes: PatternTreeNode[],
        Hierarchy: SkriptNestHierarchy
    ): PatternTreeNode[] {
        ...
        for (let i = Hierarchy.start; i < Hierarchy.end; i++) {
            ...
            if (char == "(") {
                ...
                let dataByMergedNode: Map<PatternTreeNode, MergedNodeData> = new Map<
                    PatternTreeNode,
                    MergedNodeData
                >();
                ...
                for (
                    ...
                    splitNodeIndex < currentNodes.length;
                    splitNodeIndex++
                ) {
                    ...
                    const addNode = (
                        children: Map<string, PatternTreeNode>,
                        key: string,
                        nodeFunc: () => PatternTreeNode
                    ) => {
                            ...
                            for (const possibleType of typeState.possibleTypes) {
                                ...
                                if (possibleType.section) {
                                    ...
                                    if (!typeState.staticOnly)
                                        addNode(
                                            currentSplitNode.instanceTypeChildren,
                                            possibleType.skriptPatternString,
                                            () =>
                                                new TypeNode(
                                                    possibleType.section as SkriptTypeSection
                                                )
                                        );
                                    addNode(
                                        currentSplitNode.staticTypeChildren,
                                        possibleType.skriptPatternString,
                                        () =>
                                            new TypeNode(possibleType.section as SkriptTypeSection)
                                    );
                                }
                            }
                        }
                    ...
                    //string node
                        ...
                        addNode(
                            currentSplitNode.stringOrderedChildren,
                            char,
                            () => new StringNode(char)
                        );
                    }
    ...
    static createHierarchy(context: SkriptContext): SkriptNestHierarchy {
                ...
                if (closingBraces.includes(char)) {
                    if (node.delimiter != "<" || char == ">") {
                        ...
                        if (node.delimiter != linkedOpenbrace) {
                            const oldNode = node;
                            node = hierarchy.getActiveNode();
                            if (oldNode.delimiter == "(") {
                                //this was a literal brace
        ...
        if (lastActiveNode != hierarchy) {
            context.addDiagnostic(
                lastActiveNode.start,
                hierarchy.end - lastActiveNode.start,
                "no matching closing character found",
                DiagnosticSeverity.Error,
                "IntelliSkript->Nest->No Matching"
            );
        }
        return hierarchy;
    }
        ...
        for (let i = 0; i < hierarchy.children.length; i++) {
            ...
            if (node.delimiter == "(") {
                if (currentString[node.end + ")".length] == "?") {
                    ...
                    if (hasSpaceLeft) {
                        fixedString +=
                            currentString.substring(lastFixIndex, node.start - " (".length) + "( ";
                        fixedNode = true;
                        ...
                        if (hasSpaceRight) {
                            if (lastLendCheck != spaceCheckPosition)
                                canLendSpaceRight = hasSpaceLeft; //update
    ...
    static parsePattern(
        ...
        patternSection: ReflectPatternContainerSection,
        type: PatternType
        ...
        if (!context.parseResult.diagnostics.length) {
            ...
            while ((m = argumentRegExp.exec(context.currentString))) {
                ...
                if (m.index != undefined) {
                    ...
                    context.addToken(
                        TokenTypes.pattern,
                        previousTokenEndPos,
                        typeStart - previousTokenEndPos,
                        TokenModifiers.definition
                    );
                    const result = context.currentSection.parseTypes(
                        context,
                        typeStart,
                        typeString.length
                    );
                    if (result) {
                        ...
                        context.addDiagnostic(
                            typeStart,
                            typeString.length,
                            "this type is not recognized",
                            DiagnosticSeverity.Error,
                            "IntelliSkript->Type->Not Recognized"
                        );
                        const obj = context.currentSection.getTypeData("object");
                        if (obj) {
                            //we expect the 'object' type to always be available
            ...
            try {
                ...
                const data = new PatternData(
                    ...
                    fixedString,
                    context.getLocation(0, context.currentString.length),
                    type,
                    patternSection,
                    expressionArguments,
                    argumentPositions
    ...
    addPattern(pattern: PatternData) {
        ...
        //etc.
        if (
            !PatternTree.isRegexPattern(pattern) ||
