...
export class AddonParser extends Parser {
    ...
    static parseFileJson(file: fileJson): string {
        ...
        function defineType(elem: TypeJson, parents?: string): string {
            ...
            if (parents) str += `\tinherits: ${parents}\n`;
            if (elem.usage) {
                ...
                if (patterns) {
                    ...
                    expressionString +=
                        "\treturn type: " + AddonParser.PatternToCall(elem.patterns[0]) + "\n";
        ...
        this.inheritanceByID.forEach((parents: string, name: string) => {
            if (parents != "predefined") {
                ...
                if (type) str += defineType(type, parents);
    ...
    static override ParseFiles(): string {
        ...
        const text = fs
            .readFileSync(path.join(this.parserDirectory, "inheritance.txt"), "utf8")
            .toLocaleLowerCase();
        for (const line of text.split("\n")) {
            const parts = line.trim().split("#")[0].split("->");
            if (parts.length > 1) this.inheritanceByID.set(this.normalizeName(parts[0]), parts[1]);
        }
