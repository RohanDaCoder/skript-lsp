...
export class SkriptContext {
    ...
    push(
        newPosition: number,
        newSize: number = this.currentString.length - newPosition
    ): SkriptContext {
    ...
    addToken(
        type: TokenTypes,
        relativePosition = 0,
        length = this.currentString.length - relativePosition,
        ...modifiers: TokenModifiers[]
    ): void {
        const absolutePosition = this.currentDocument.positionAt(
            this.currentPosition + relativePosition
        );
        this.parseResult.tokens.push(
            new SemanticToken(absolutePosition, length, type, SemanticToken.modToFlags(modifiers))
        );
    }
        ...
        this.addDiagnosticAbsolute(
            {
                ...
                end: this.currentDocument.positionAt(
                    this.currentPosition + relativePosition + length
                ),
            },
            message,
            severity,
    ...
    getHierarchy(addDiagnostics = false) {
            ...
            if (currentChar == '"') {
                ...
                if (node.delimiter == '"') {
                    if (this.currentString[i + 1] == '"') {
                        i++;
                        continue; //skip escaped string characters
                    } else {
                        node.end = i; //pop
                    }
                ...
                if (node.delimiter == "%") {
                    ...
                    if (i == 0 || this.currentString[i - 1].match(/[0-9]/) == null) {
                        //don't push for percentages
                        node.children.push(new SkriptNestHierarchy(i + 1, "%")); //push
                    }
                }
                ...
                if (node.delimiter == "(")
                    ...
                    this.hierarchy
                        .getActiveNode()
                        .children.push(new SkriptNestHierarchy(i + 1, ","));
                }
            } else if (openBraces.includes(currentChar)) {
                const node = this.hierarchy.getActiveNode();
                if (node.delimiter != '"') {
                    //braces don't count in a string
                ...
                if (node.delimiter != '"') {
                    ...
                    if (node.delimiter == "," && currentChar == ")") {
                        ...
                        node.delimiter.length &&
                        closingBraces.indexOf(currentChar) == openBraces.indexOf(node.delimiter)
                    ) {
                        node.end = i; //pop
                        ...
                        this.addDiagnostic(
                            i,
                            1,
                            "unmatched closing character found",
                            DiagnosticSeverity.Error,
                            "IntelliSkript->Nest->UnMatched"
                        );
                    }
                }
            }
        ...
        while (true) {
            const lastActiveChildNode: SkriptNestHierarchy | undefined =
                lastActiveNode.getActiveChildNode();
            if (!lastActiveChildNode) break;
            if (addDiagnostics) {
                if (lastActiveChildNode != this.hierarchy) {
                    this.addDiagnostic(
                        lastActiveChildNode.start,
                        this.hierarchy.end - lastActiveChildNode.start,
    ...
    splitHierarchically(
        ...
        for (let i = 0; i < indexes.length; i++) {
            results[i] = {
                text: this.currentString.substring(currentIndex, indexes[i].index),
                index: currentIndex,
            };
            currentIndex = indexes[i].index + indexes[i][0].length;
        }
        results[indexes.length] = {
            text: this.currentString.substring(currentIndex, end),
            index: currentIndex,
        };
        return results;
    }
    ...
    addPatternMatch(data: PatternData, start = 0, end = this.currentString.length) {
        this.currentSkriptFile?.matches.addNestedChild(
            new SkriptPatternMatchHierarchy(
