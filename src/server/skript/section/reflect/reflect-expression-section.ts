...
export class ReflectExpressionSection extends ReflectPatternContainerSection {
    ...
    createSection(context: SkriptContext): SkriptSection | undefined {
        ...
        switch (context.currentString) {
            ...
            //set %type%
            default:
                if (context.currentString.startsWith("set ")) {
    ...
    processLine(context: SkriptContext): void {
        if (context.currentString.startsWith("return type: ")) {
            ...
            let parsedType =
                this.parseType(context, "return type: ".length) ?? this.getTypeData("object");
            if (parsedType) {
            ...
            let parsedType =
                this.parseType(context, "loop of: ".length) ?? this.getTypeData("object");
    ...
    override finish(context: SkriptContext): void {
        if (this.returnType.possibleTypes.length == 0) {
            ...
            if (unknownData) this.returnType.possibleTypes.push(unknownData);
        }
