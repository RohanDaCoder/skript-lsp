    ...
    processLine(context: SkriptContext): void {
        if (context.currentString.startsWith("event-values: ")) {
            ...
            for (let i = 0; i < valueStrings.length; i++) {
                const eventValueType = this.parseType(
                    context,
                    currentPosition,
                    valueStrings[i].length
                );
                if (eventValueType) {
                    this.eventValues.push(
                        new PatternData(
                            "[the] [event( |-)]]" + valueStrings[i],
                            "(the )?(event( |-))?" + valueStrings[i],
                            context.getLocation(currentPosition, valueStrings[i].length),
                            PatternType.expression,
                            this,
                            [],
                            [],
                            new SkriptTypeState(eventValueType)
                        )
                    );
                }
