...
export class SkriptSection extends SkriptSectionGroup {
    ...
    getTypeData(typeName: string): TypeData | undefined {
        return this.getScope()?.getPatternMatch(new SkriptPatternCall(typeName, [PatternType.type]))
            ?.fullMatch.matchedPattern;
    }
    getParentSection(): SkriptSection | undefined {
        return this.parent && this.parent instanceof SkriptSection ? this.parent : undefined;
    }

    /**expects start and length! */
    parseType(
        context: SkriptContext,
        start = 0,
        length = context.currentString.length - start
    ): TypeData | undefined {
        ...
        context.addToken(
            ...
            start,
            length,
            ...(data ? [] : [TokenModifiers.deprecated])
        );
        return data;
    }
     ...
     */
    private tokenizeMatch(
        context: SkriptContext,
        pattern: TransformedPattern,
        match: PatternData,
        matchPatternStart: integer = 0,
        matchPatternEnd: integer = pattern.pattern.length
    ) {
        const tokenType: TokenTypes =
            TokenTypes[PatternType[match.patternType] as keyof typeof TokenTypes]; //match.section instanceof SkriptPropertySection ?
        ...
        if (matchPatternEnd > tokenizeFrom)
            ...
            context.addToken(
                tokenType,
                pattern.getLinePos(tokenizeFrom),
                matchPatternEnd - tokenizeFrom
            );
    }

    /**visualizes matches recursively */
    private visualizeMatch(
        context: SkriptContext,
        pattern: TransformedPattern,
        ...
        context.addPatternMatch(
            currentMatch.matchedPattern,
            pattern.getLinePos(currentMatch.start),
            pattern.getLinePos(currentMatch.end)
        );
    }

    //detect patterns like a [b | c]
    //return value: a type. basically, it will convert each subpattern into a result type (a %)
    detectPatternsRecursively(
        context: SkriptContext,
        mainPatternTypes: PatternType[] = [PatternType.effect, PatternType.condition],
        isTopNode = true,
        currentNode: SkriptNestHierarchy = context.getHierarchy(true)
    ): { detectedPattern: PatternData | undefined } {
        let foundPattern: PatternData | undefined;
        const mergedPatternArguments: Map<number, SkriptTypeState> = new Map<
            number,
            SkriptTypeState
        >();
        ...
        for (let i = 0; i < currentNode.children.length; i++) {
            ...
            const childResults = this.detectPatternsRecursively(
                context.push(nodeToClone.start, nodeToClone.end - nodeToClone.start),
                [PatternType.expression],
                false,
                offsetNode
            );
            if (childResults.detectedPattern) childResultList[i] = childResults.detectedPattern;
        }
        ...
        if ("%(".includes(currentNode.delimiter)) {
            ...
            for (let i = 0; i < currentNode.children.length; i++) {
                ...
                if ('"{('.includes(child.delimiter)) {
                    //string or variable
                    let typeToReplace: SkriptTypeState | undefined;
                    if (child.delimiter == "(") {
                        if (childResultList[i]) typeToReplace = childResultList[i].returnType;
                        else {
                            //check if this is a function
                            //search to the left (to where the name would end)
                            const functionNameRegex =
                                /(?:([a-zA-Z_]{1,})\.)?([a-zA-Z_][a-zA-Z0-9_]{1,})$/g;
                            const functionNameEnd = child.start - 1;
                            let match;
                            if (
                                (match = functionNameRegex.exec(
                                    context.currentString.substring(0, functionNameEnd)
                                ))
                            ) {
                                ...
                                if (javaClass) {
                                    ...
                                    let matchingFunction =
                                        this.getScope()?.getMatchingFunction(functionName);
                                    if (!matchingFunction)
                        ...
                        //variable
                        this.addVariableReference(
                            context.getLocation(child.start, child.end - child.start),
                        ...
                        if (stringData) typeToReplace = new SkriptTypeState(stringData);
                    }
                    if (!typeToReplace) {
                    ...
                    if (typeToReplace) mergedPatternArguments.set(child.start, typeToReplace);
            ...
            if (
                currentPatternArguments.length == 1 &&
                ...
                while ((nonWhiteSpaceMatchArray = doubleSpacesRegex.exec(pattern.pattern))) {
                    ...
                    context.addDiagnostic(
                        start,
                        pattern.getLinePos(
                            nonWhiteSpaceMatchArray.index + nonWhiteSpaceMatchArray[0].length
                        ) - start,
                        "Expected a single space here",
                        DiagnosticSeverity.Warning,
                        "IntelliSkript->Pattern->WhiteSpace"
                    );
                }
                let matchResult = undefined;
                const call = new SkriptPatternCall(
                    pattern.pattern,
                    mainPatternTypes,
                    currentPatternArguments
                );
                context.parseResult.patternsParsed.push([
                    call,
                    currentNode.cloneWithOffset(context.currentPosition),
                ]);
                ...
                if (!matchResult && isTopNode) {
                    context.addDiagnostic(
                        ...
                        "can't understand this line (pattern detection is a work in progress. please report on discord)",
                        DiagnosticSeverity.Hint,
                        "IntelliSkript->Pattern"
                    );
                }
            }
        }
        //won't pass for '' because it's being handled above
            ...
            const tokenType =
                currentNode.delimiter == '"' ? TokenTypes.string : TokenTypes.variable;
                    ...
                    for (let index = start; index < end; index++) {
                        ...
                        if (currentChar == "&" && index + 1 < end) {
                            ...
                            if (/[0-9a-fl-or]/.test(nextChar)) {
                                ...
                                if (nextChar == "r") {
                                    ...
                                    const newModifier =
                                        TokenModifiers[
                                            ("bukkit_" + nextChar) as keyof typeof TokenModifiers
                                        ];
                        ...
                        //double #
                            ...
                            context.addDiagnostic(
                                index,
                                2,
                                "double #s don't get replaced to single #s anymore",
                                DiagnosticSeverity.Warning,
                                "IntelliSkript->Nest->Double Hashtags"
                            );
                        }
                    }
        ...
        while ((p = checkPattern.exec(context.currentString))) {
            ...
            if (node && node.start == braceEndIndex) {
                context.addDiagnostic(
                    ...
                    "IntelliSkript->Performance->Braces->Lambda"
                );
                //isIfStatement = true;
            }
        }
