...
export class Scope {
    ...
    constructor(parent?: Scope) {
        ...
        if (parent) this.containersToTraverse.push(...parent.containersToTraverse);
        this.containersToTraverse.push(this);
    }

    testTypeNodes(
        testChild: (testChild: PatternTreeNode) => void,
        typeNodes: Map<string, PatternTreeNode>,
        possibleTypes: SkriptTypeState
    ) {
        ...
        for (const type of possibleTypes.possibleTypes) {
            if (type.skriptPatternString == "unknown" || type.skriptPatternString == "object[s]") {
                ...
                if ((type.section as SkriptTypeSection).testBaseClasses(testClass, testedTypes))
                    return;
            }
        }
        //return testAllTypes;
        ...
        } else if (!testedTypes.has("object[s]")) testClass("object[s]");
    }
    ...
    canSubstitute(progress: MatchProgress) {
        ...
        return (
            canHaveSubPattern(progress.patternType) &&
    ...
    stepTreeNode(progress: MatchProgress): MatchProgress[] {
        ...
        const checkEndNode = (endNodeData: PatternData, checkIndex = progress.index) => {
            ...
            if (checkIndex == pattern.length && !progress.parent) {
                ...
                const testParentNode = (parentProgress: MatchProgress) => {
                    const childMatch = new PatternMatch(
                        progress.start,
                        checkIndex,
                        endNodeData,
                        progress.subMatches
                    ...
                    const testTypeChild = (testChild: PatternTreeNode) => {
                        ...
                        nodesToCheck.push({
                            ...cloneProgress(parentProgress),
                            subMatches: parentProgress.subMatches.concat([childMatch]),
                            currentNode: testChild,
                            rootNode: testChild,
                            index: checkIndex,
                            argumentIndex: progress.argumentIndex,
                        });
                    };
                    if (progress.patternType == PatternType.expression) {
                        this.testTypeNodes(
                            testTypeChild,
                            parentProgress.currentNode.instanceTypeChildren,
                            endNodeData.returnType
                        );
                    } else {
                        //we don't have to check for both, as all statictypechildren also exist on instancetypechildren
                        this.testTypeNodes(
                            testTypeChild,
                            parentProgress.currentNode.staticTypeChildren,
                            new SkriptTypeState(endNodeData as TypeData)
                        );
                    }
        ...
        if (
            progress.currentNode == progress.rootNode &&
            progress.patternType == PatternType.expression &&
            /[0-9-]/.test(pattern[progress.index])
        ) {
            for (let [key, child] of progress.currentNode.regExpOrderedChildren) {
                ...
                if (matchArray) {
                    nodesToCheck.push({
                        ...
                        currentNode: child,
                        index: progress.index + matchArray[0].length,
                    });
                }
            }
        }
        ...
        if (progress.index < pattern.length) {
            ...
            if (this.canSubstitute(progress)) {
                for (const substitutablePatternType of SubstitutablePatterns) {
                    if (
                        substitutablePatternType == PatternType.type
                            ? hasValidStaticNodes
                            : hasValidInstanceNodes
                    ) {
                        for (const container of this.containersToTraverse) {
                            //clone
                            const root =
                                container.trees[substitutablePatternType].compileAndGetRoot();
                            nodesToCheck.push({
                                ...progress,
                                start: progress.index,
                                parent: { ...cloneProgress(progress) },
                                currentNode: root,
                                rootNode: root,
                                patternType: substitutablePatternType,
                            });
                        }
                    }
                }
            ...
            if (progress.currentNode.instanceTypeChildren.size) {
                ...
                const testClass = (typeChild: PatternTreeNode): void => {
                    if (typeChild) {
                        nodesToCheck.push({
                            ...cloneProgress(progress),
                            currentNode: typeChild,
                            rootNode: progress.currentNode,
                            index: newIndex,
                ...
                if (
                    ...
                    const currentArgument =
                        progress.testPattern.expressionArguments[progress.argumentIndex];
                    newIndex++;
                    newArgumentIndex++;
                    this.testTypeNodes(
                        testClass,
                        progress.currentNode.instanceTypeChildren,
                        currentArgument
                    );
                }
            }
    ...
    getMatchingPatternPart(startNodes: MatchProgress[]): MatchResult | undefined {
        ...
        let nextStepsToTake: (richProgress | undefined)[] = startNodes.map((node) => {
            return { progress: node, nodesPassed: [node.currentNode] };
        });
        ...
        while ((currentProgress = nextStepsToTake.pop())) {
            ...
            nextStepsToTake.push(
                ...nextSteps.map((value) => {
                    return {
                        progress: value,
                        nodesPassed: [
                            ...(currentProgress as richProgress).nodesPassed,
                            value.currentNode,
                        ],
                    };
                })
