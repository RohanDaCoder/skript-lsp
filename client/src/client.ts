...
const connectionString =
    "InstrumentationKey=3beed08d-7b3d-40cd-a129-05ff7cb7b2f9;IngestionEndpoint=https://westeurope-5.in.applicationinsights.azure.com/;LiveEndpoint=https://westeurope.livediagnostics.monitor.azure.com/;ApplicationId=bfda7f12-a9c2-4196-bd99-850fa9f29864";
...
export async function startClient(
    ...
    const myProvider = new (class implements TextDocumentContentProvider {
        provideTextDocumentContent(uri: Uri): string {
            //remove the '\'
            return (
                resourceFiles.get(uri.path.substring((resourceFolderString + "/").length)) ??
...
export async function activateClient(context: ExtensionContext, client: BaseLanguageClient) {
    ...
    const readFileListener = client.onRequest(
        "custom/readFile",
    ...
    const listFilesListener = client.onRequest(
        "custom/listFiles",
        async (params: { folderUri: string }) => {
    ...
    const getDocumentsListener = client.onRequest(
        "custom/getDocuments",
        async (params: { folderUri: string }) => {
            const folderUri = Uri.parse(params.folderUri);
                ...
                for (const file of resourceFiles) {
                    fileList.push({
                        uri: Uri.from({
                            ...
                            path: resourceFolderString + "/" + file[0],
                        }).toString(),
                        content: file[1],
                    });
                }
    ...
    window.onDidChangeActiveTextEditor((editor) => {
        if (editor && editor.document) {
            client.sendNotification("custom/onDidChangeActiveTextEditor", {
